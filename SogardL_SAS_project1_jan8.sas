...
DATE: Thursday, January 08, 2015     TIME: 9:24:06 AM
PROJECT: SogardL_SAS_project1_jan8
PROJECT PATH: P:\QAC\qac200\students\lsogard\Assignments\SogardL_SAS_project1_jan8.egp
---------------------------------------- */

/* Library assignment for Local.MYDATA */
Libname MYDATA BASE 'P:\QAC\qac200\students\lsogard' ;
/* Library assignment for Local.MYDATA */
Libname MYDATA BASE 'P:\QAC\qac200\students\lsogard' ;
...
%LET _CLIENTPROJECTPATH='P:\QAC\qac200\students\lsogard\Assignments\SogardL_SAS_project1_jan8.egp';
%LET _CLIENTPROJECTNAME='SogardL_SAS_project1_jan8.egp';
...
/* -------------------------------------------------------------------
   ...
   Generated on: Thursday, January 08, 2015 at 9:23:57 AM
...
/*   START OF NODE: Filter and Sort1   */
%LET _CLIENTTASKLABEL='Filter and Sort1';
%LET _CLIENTPROJECTPATH='P:\QAC\qac200\students\lsogard\Assignments\SogardL_SAS_project1_jan8.egp';
%LET _CLIENTPROJECTNAME='SogardL_SAS_project1_jan8.egp';

GOPTIONS ACCESSIBLE;
%_eg_conditional_dropds(MYDATA.'18OvrVariables'n);

PROC SQL;
   CREATE TABLE MYDATA.'18OvrVariables'n(label="18OvrVariables") AS 
   SELECT t1.ADAPPT42, 
          t1.ADCAPE42, 
          t1.ADCLIM42, 
          t1.ADCMPD42, 
...
PROC DATASETS NOLIST NODETAILS; 
   CONTENTS DATA=MYDATA.'18OVRVARIABLES'n OUT=WORK.SUCOUT1;

RUN;

DATA MYDATA.CONTCONTENTSFOR18OVRVARIABLES(LABEL="Contents Details for 18OVRVARIABLES");
   SET WORK.SUCOUT1;
RUN;
...
CREATE VIEW WORK.SCVIEW AS 
	SELECT DISTINCT memname LABEL="Table Name", 
			...
			charset LABEL="Char. Set", 
			protect LABEL="Password Protected", 
			typemem LABEL="Data Set Type" FROM MYDATA.CONTCONTENTSFOR18OVRVARIABLES
	ORDER BY memname ; 

CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.SCVIEW
		WHERE memname='18OVRVARIABLES';
QUIT;
...
PROC SORT DATA=MYDATA.CONTCONTENTSFOR18OVRVARIABLES OUT=MYDATA.CONTCONTENTSFOR18OVRVARIABLES;
   BY memname name;
RUN;
...
DROP TABLE WORK.SCTABLE;
CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM MYDATA.CONTCONTENTSFOR18OVRVARIABLES
		WHERE memname='18OVRVARIABLES';
QUIT;
